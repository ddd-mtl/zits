...
pub fn generate_typescript_bindings(
    ...
    if can_proxy {
        /// Append header
        let copy = state.zome_proxy_output.clone();
        state.zome_proxy_output.clear();
        state.write_zome_proxy_header(&zome_name, &default_zome_name);
        state.zome_proxy_output.push_str(&copy);
