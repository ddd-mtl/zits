...
macro_rules! check_attributes {
    ($x: ident, $a: ident, in: $y: tt, $z: tt, $debug: ident) => {
        let has_zits_attribute = has_zits_attribute(&$a.attrs);
        if $debug {
            if has_zits_attribute {
                println!("[zits][debug] Encountered {}: {}", $y, $x.ident.to_string());
            } else {
                println!("[zits][debug] Encountered non-zits {}: {}", $y, $x.ident.to_string());
            }
        }
...
///
fn has_zits_attribute(attributes: &Vec<syn::Attribute>) -> bool {
